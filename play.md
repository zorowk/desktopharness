...
- [ ] 至少选取 5 个可重复桌面任务并完成每项 10 次重复验证：1/5 受约束精确鼠标移动已 10/10；3/5、4/5、5/5 只有重复样本，未做严格 10 连跑。输入任务的汇总分母/分子在旧记录中不一致，须由原始证据核对后重跑或更正，当前不计入通过。
...
- [ ] 重复任务 2/5（空白文档固定文本输入）：剪贴板字节级后置条件的方案已使用；两次运行分别记录为 9/10 和 8/10 输入精确，但旧记录同时写为“18 次采样 17/18”，汇总不自洽。需保留原始剪贴板证据后重新汇总，再决定是否通过；文档未保存，session 均已 reset。
...
- [x] 每次动作后重新采集截图和树，计算窗口、焦点、标题和 geometry 的前后差异：`qwen_cua_execute` 的 `post_validation` 现记录后置截图摘要、活动窗口、可操作窗口数、目标 geometry 与身份状态；后置观察不可用、目标消失或身份歧义会令 session 安全重置。已补单元回归；真实链路复验待补录。
