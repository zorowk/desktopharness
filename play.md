...
- [ ] 收集真实 Qwen 返回的动作文本，覆盖点击、双击、右击、输入、滚动、快捷键、等待和完成等类型；已取得点击、双击、右击、`Esc`、等待和完成样本，输入未测，滚动请求未生成合法滚动动作。
...
- [ ] 扩大真实动作样本，补齐输入、合法滚动和其他安全快捷键；当前真实样本已覆盖移动、点击、双击、右击、`Esc`、等待和完成，但部分动作的任务结果未通过。
...
- [ ] 滚动动作失败：模型在约 53k tokens 的异常响应中重复生成至少 80 个相同 `moveTo`，没有生成滚轮动作；未执行并 reset session。需要限制模型最大输出、限制单提案动作数量并拒绝高度重复动作。
- [ ] 双击任务失败：`doubleClick` 注入层报告成功，但后置截图仍只有桌面和 `htop`，没有测试页面或新应用窗口；Qwen 仍依据历史宣告完成。需要把执行器成功与任务后置条件成功分开，并增加基于新观察/树变化的验证。
- [ ] 右击语义目标失败：模型声称右击 `cua-test.html`，实际出现包含 `Mode`、`Position`、`Lock the Dock` 等条目的 Dock 菜单。Treeland 只能把坐标归到 `BackgroundContainer`，无法在桌面图标与 Dock 子控件之间做语义校验。
- [ ] MCP 端到端等待仍有大幅抖动：后端预测 telemetry 通常约 0.9～2.2 秒，但外层调用观察到约 55、74、123 和 163 秒等待；该问题与模型推理耗时和动作复杂度不成正比，需要为传输、截图、取树、反馈和响应序列化分别计时。
- [ ] 在打开专用空白编辑器前不测试文字输入；当前活动窗口是运行 `htop` 的终端，向其中输入不符合安全验收条件。

### 2.6 低风险执行

- [x] 使用移动鼠标动作开始调用 `qwen_cua_execute`，未执行点击或输入。
- [x] 执行桌面测试图标单击、等待和 `Esc` 等低风险动作并完成下一轮观察。
- [ ] 在专用测试窗口继续测试点击和滚动；桌面右击暴露了 Dock/桌面图标语义归属不足，滚动预测暴露了重复动作失控。
...
- [ ] 修正执行坐标空间：真实关闭按钮移动暴露 `qwen_cua_execute` 将 Treeland 逻辑坐标直接传给 Wayland `ydotool`，在 1.25 缩放下少乘缩放系数。首次修复将逻辑坐标反转换为截图像素后，真实执行尝试 `(1566,100)` 却读回鼠标 `(1535,200)`：x 被裁剪至 Treeland 右边界且 y 存在额外缩放，说明 Wayland `ydotool` 的运行时输入空间不同于截图像素。保留读回位置，下一步需先标定输入空间再决定转换公式。
